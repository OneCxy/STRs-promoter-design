# python /root/autodl-tmp/cxy/CODE/STR/Generator/step2_cGAN.py
...
from Generator.pro_data1 import LoadData
...
# from Predictor.predictor_training import Seq2ScalarTraining
import matplotlib
import numpy as np
...
def main():
    ...
    train_data, test_data = DataLoader(LoadData(is_train=True, path='/root/autodl-tmp/cxy/CODE/STR/cachetest1/fillseq_20260203095533_with_loss_tiqu.csv', split_r=1),
                                       batch_size=32, shuffle=True), DataLoader(LoadData(is_train=True, path='/root/autodl-tmp/cxy/CODE/STR/cachetest1/fillseq_20260203095533_with_loss_tiqu.csv', split_r=1), batch_size=32)
    train_data = get_infinite_batches(train_data)
    logger = utils.get_logger(log_path='/root/autodl-tmp/cxy/CODE/STR/cachetest2/training_log/', name=data_name)
    n_critics = 5
    n_iters = 5000
    ...
    for i in range(n_iters):
        ...
        if i % 100 == 99:
            ...
            d_loss, g_loss, g_l1 = 0, 0, 0
            csv_name = save_sequence(tensorSeq, tensorInput, tensorRealB, save_path='/root/autodl-tmp/cxy/CODE/STR/cachetest2/', name='inducible_{}_'.format(data_name), cut_r=1)
            utils.kmer_frequency(csv_name, '/root/autodl-tmp/cxy/CODE/STR/cachetest1/fillseq_20260203095533_with_loss_tiqu.csv', k=4, save_path='/root/autodl-tmp/cxy/CODE/STR/cachetest2/figure/', save_name=data_name + str(i))
            csv2fasta(csv_name, '/root/autodl-tmp/cxy/CODE/STR/cachetest2/gen_', 'iter_{}'.format(i))
            # torch.save(model.generator, 'check_pointstest2/' + data_name + '_net_G_' + str(i) + '.pth')
            # torch.save(model.discriminator, 'check_pointstest2/' + data_name + '_net_D_' + str(i) + '.pth')
    save_dir = '/root/autodl-tmp/cxy/CODE/STR/check_pointstest2/'
